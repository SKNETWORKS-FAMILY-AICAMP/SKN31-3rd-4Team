...
"""
from state import ExtractionResult, Citation
from .personal_graph_db import PersonalGraphDB
from .medical_graph_db import MedicalGraphDB
...
class HybridRetriever:
    ...
    def _merge_to_context(
        ...
        if medical_subgraph:
            lines.append("\n[의학 그래프 - 약물/질병/논문 근거]")
            records = medical_subgraph.get("records") if isinstance(medical_subgraph, dict) else None
            if isinstance(records, list):
                for r in records:
                    lines.append(f"- {r.get('drug','-')}: 치료 대상 = {', '.join(r.get('diseases', []))}")
                    for se in r.get("side_effects", []):
                        lines.append(f"  · 부작용: {se}")
                    for inter in r.get("interactions", []):
                        lines.append(f"  · 상호작용: {inter}")
                    for p in r.get("papers", []):
                        lines.append(f"  · 근거 논문: {p}")
                        citations.append(Citation(source_graph="medical", node_or_doc=p))
            else:
